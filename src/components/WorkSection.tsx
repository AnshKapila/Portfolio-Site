...
export function WorkSection() {
  ...
  const featuredProjects = projects.filter((p) =>
    featuredProjectsSlugs.includes(p.slug),
  );

  return (
    <section
      id="work"
      className="w-full relative bg-black text-white px-6 md:px-12 lg:px-16 pt-24 pb-48 border-t border-white/5"
    >
                  ...
                  <div className="relative z-10 p-8 md:p-10 lg:p-12">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {project.tags.map((tag) => (
                        <span
                          key={tag}
                          className="px-4 py-2 rounded-lg bg-[#F24E1E]/10 border border-[#F24E1E]/20 text-xs font-semibold tracking-wide text-[#FFD1C2]/90 shadow-sm backdrop-blur-sm"
                        >
                          {tag}
                        </span>
                      ))}
