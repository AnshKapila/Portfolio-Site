...
import { 
  ArrowUpRight, Smartphone, Cpu, Code, Search, Layers, Compass, Figma, 
  CheckSquare, Paintbrush, Workflow, Zap, FileText, TrendingUp, Sparkles, 
  ChevronDown, ChevronUp, Check, X, Maximize2, HelpCircle 
} from 'lucide-react';
...
interface ServiceDetail {
  ...
  processSteps: { title: string; desc: string; duration: string }[];
  faqs: FAQItem[];
  bentoFeatures: { title: string; desc: string; icon: string; value?: string }[];
  modelsTitle: string;
  models: { name: string; desc: string; speed: string; target: string; icon: string }[];
}
  ...
  if (!service) {
    return (
      <div className="pt-32 pb-48 px-6 min-h-screen text-center flex flex-col items-center justify-center">
        ...
        <Link to="/" className="mt-8 text-[#F24E1E] hover:underline hover:text-[#FF6A4D] transition-colors" id="return-home-link">
          Return Home
        </Link>
      </div>
    );
    ...
    <>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        ...
        {service.slug === 'ai-content-growth-systems' && (
          <div className="absolute top-[30%] -left-40 w-[400px] h-[400px] rounded-full bg-purple-500/5 blur-[100px] opacity-50" />
        )}
      </div>
      ...
      <div className="relative pt-32 pb-48 md:pt-40 px-6 md:px-12 lg:px-16 min-h-screen z-10" id="services-main-container">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-12 flex items-center justify-between border-b border-white/5 pb-6" id="service-navbar-sub">
            <Link 
              to="/services" 
              className="group inline-flex items-center gap-2 text-xs font-mono text-gray-500 hover:text-white uppercase tracking-wider transition-colors"
              ...
              {details.bentoFeatures.map((feat, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`p-8 rounded-3xl border transition-all duration-300 group flex flex-col justify-between ${
                      isPromo 
                        ? 'md:col-span-2 border-white/15 bg-white/[0.02] hover:bg-white/[0.04]' 
                        : 'md:col-span-1 border-white/5 bg-zinc-900/30 hover:border-white/10'
                    } ${themeAccents.cardBorder}`}
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between mb-6">
                        ...
                        {feat.value && (
                          <span className="text-xs font-mono text-[#F24E1E]/90 bg-[#F24E1E]/5 px-3 py-1 rounded-full border border-[#F24E1E]/10">
                            {feat.value}
                          </span>
                        )}
                      </div>
          ...
          {showcaseImages.length > 0 ? (
            <div className="mb-32 animate-fade-in" id="visual-showcase-section">
              <FadeIn delayMs={100} durationMs={800}>
                    ...
                    return (
                      ...
                      >
                        <img
                          ...
                          alt={`${service.title} Portfolio Item ${i + 1}`}
                        ...
                        {i !== 1 && (
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                            ...
                            <div className="text-center p-4">
                              ...
                              {i !== 14 && (
                                <span className="text-sm font-sans text-white font-medium">Click to Inspect</span>
                              )}
                            </div>
                          </div>
                        )}
          ...
          <div className="mb-32" id="engagement-models-section">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {details.models.map((model, idx) => (
                <div 
                  key={idx}
                  className="p-8 md:p-10 rounded-3xl border border-white/5 bg-[#111]/40 flex flex-col justify-between group hover:border-[#F24E1E]/20 transition-all duration-300"
                  id={`model-card-${idx}`}
                >
                  <div>
                    <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="border-t border-white/5 pt-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <Link 
                      to="/contact" 
                      className="text-xs font-mono text-[#F24E1E] uppercase tracking-wider font-semibold hover:text-[#FF6A4D] transition-colors flex items-center gap-1"
                    >
          ...
          {serviceProjects.length > 0 && (
            <div className="mb-32" id="case-studies-section">
              <FadeIn delayMs={100} durationMs={800}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
                {serviceProjects.map((project, i) => (
                  <motion.div
                    ...
                    className="group flex flex-col items-start bg-zinc-950/30 border border-white/5 rounded-3xl overflow-hidden shadow-2xl hover:border-white/10 transition-colors"
                    ...
                    <div className="p-8 md:p-10 flex flex-col flex-grow w-full">
                      <div className="flex flex-wrap gap-2 mb-6 text-[10px] font-mono font-semibold tracking-wide text-[#FFD1C2]/90">
                        {project.tags.slice(0, 3).map(t => (
                          <span key={t} className="px-2.5 py-1 rounded-md bg-[#F24E1E]/5 border border-[#F24E1E]/10">
                            {t}
                          </span>
                        ))}
                      </div>
                      ...
                      <Link 
                        ...
                        className="inline-flex items-center justify-center w-full px-8 py-3.5 bg-white text-black hover:bg-gray-200 transition-colors rounded-lg font-medium text-xs font-mono uppercase tracking-wider"
                        id={`project-btn-${project.slug}`}
                      >
                        Inspect Case Study
                      </Link>
                    </div>
            ...
            <div className="max-w-3xl mx-auto space-y-4">
              {details.faqs.map((faq, idx) => {
                ...
                return (
                  <div 
                    key={idx}
                    className="border border-white/5 rounded-2xl bg-[#111]/20 overflow-hidden transition-all duration-300"
                    id={`faq-item-container-${idx}`}
                  >
                    <button
                      onClick={() => setOpenFaqIdx(isOpen ? null : idx)}
                      className="w-full text-left p-6 flex justify-between items-center text-white focus:outline-none focus:text-[#F24E1E]"
          ...
          <FadeIn delayMs={200} durationMs={800}>
            <div 
              className="w-full bg-[#111111] border border-white/5 p-12 lg:p-20 rounded-[2.5rem] text-center flex flex-col items-center shadow-2xl relative overflow-hidden group"
        ...
        {(activeLightboxImg || activeLightboxVideo) && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" id="lightbox-wrapper">
            ...
            <motion.div
              ...
              className="relative max-w-5xl max-h-[85vh] z-10 overflow-hidden rounded-2xl border border-white/10 shadow-[0_0_80px_rgba(0,0,0,0.8)]"
              id="lightbox-body"
            >
              {/* Close marker */}
              <button
