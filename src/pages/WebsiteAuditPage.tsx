...
export function WebsiteAuditPage() {
  ...
  const [activeLeak, setActiveLeak] = useState<number | null>(0);
  ...
  const faqs = [
    ...
    },
    {
      ...
      answer: "We would love to. If you choose to hire Intent Studios to implement the structural changes or complete a full redesign after your audit, we will credit the full cost of your audit toward your redesign project."
    }
  ];

  return (
    <div className="bg-black text-white min-h-screen relative overflow-hidden" id="audit-page-root">
      <SEO 
        title="Professional Website Audit — Fix Your UX & Copy Leaks | Intent Studios"
        description="A professional, manual audit of your website's UX, trust factor, messaging, and layout leaks. Get actionable feedback to increase enquiries and conversions."
        keywords="website audit, UX audit, website review, conversion audit, website consultation, user experience audit"
        url="https://intentstudios.in/audit"
      />
            ...
            >
              {leaks.map((leak, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    onMouseEnter={() => setActiveLeak(idx)}
                    className="relative cursor-pointer group py-5"
                    id={`leak-category-btn-${idx}`}
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <h3 
                          className={`text-xl md:text-2xl font-heading italic tracking-tight transition-colors duration-300 ${
                            isActive 
                              ? "text-white" 
                    ...
                    {isActive && (
                      <motion.div
                        layoutId="activeLeakLine"
                        className="absolute bottom-0 left-0 h-[2px] bg-[#F24E1E] z-10"
                        initial={false}
                        transition={{
                          type: "spring",
                          stiffness: 300,
      ...
      <section className="py-24 px-6 md:px-12 lg:px-16 border-b border-white/5 relative z-10" id="audit-options-pricing">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {auditOptions.map((opt, i) => (
            <div 
              key={i} 
              className={`p-8 md:p-10 rounded-2xl flex flex-col justify-between transition-all duration-300 relative ${opt.popular ? 'border-2 border-[#F24E1E] bg-[#0d0402] shadow-[0_0_50px_rgba(242,78,30,0.08)]' : 'border border-white/10 bg-[#080808] hover:border-white/20'}`}
              id={`audit-pricing-card-${i}`}
            >
              {opt.popular && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#F24E1E] text-white text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full">
                ...
                <div className="space-y-4 mb-8">
                  ...
                  <ul className="space-y-3">
                    {opt.features.map((feat, fIdx) => (
                      <li key={fIdx} className="flex items-start gap-2.5 text-zinc-300 text-sm font-light leading-snug">
      ...
      <section className="py-24 px-6 md:px-12 lg:px-16 border-b border-white/5 bg-zinc-950/40 relative z-10" id="audit-how-it-works">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8 relative">
          ...
          {[
            ...
            <div key={idx} className="text-center group" id={`how-it-works-step-${idx}`}>
      ...
      <section className="py-24 px-6 md:px-12 lg:px-16 border-b border-white/5 bg-zinc-950/20 relative z-10" id="audit-sample-findings">
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          {sampleFindings.map((finding, idx) => (
            <div 
              key={idx} 
              className="p-6 rounded-2xl border border-white/5 bg-[#080808] flex flex-col md:flex-row gap-6 justify-between items-start md:items-center relative overflow-hidden"
              id={`sample-finding-card-${idx}`}
            >
          ...
          <div className="flex flex-col gap-4 lg:w-2/3">
            {faqs.map((faq, i) => {
              ...
              return (
                <div 
                  key={i} 
                  className={`border border-white/10 rounded-xl overflow-hidden transition-colors duration-300 cursor-pointer select-none ${isOpen ? "bg-white/[0.04]" : "bg-transparent hover:bg-white/[0.02]"}`}
                  onMouseEnter={() => {
                    if (!isMobile) {
                      setActiveFaq(i);
                    }
